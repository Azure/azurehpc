...
![Build Status](https://azurecat.visualstudio.com/hpccat/_apis/build/status/azhpc/examples/cc_lustre?branchName=master)

Here we will explain how to deploy a full system with a VNET, JUMPBOX, CYCLESERVER and LUSTRE by using building blocks.
...
```json
{
  "variables": {
    ...
    "lustre_storage_account": "locker{{variables.uuid}}",
...
To start the PBS cluster attached to Lustre:
...
To start the Slurm cluster attached to Lustre:
...
## Step 2 - Check that Lustre and NFS shared are mounted
...
```
[hpcadmin@ip-0A020804 ~]$ qsub -N ior -k oe -j oe -l select=1:ncpus=60:mpiprocs=60 -- /apps/ior/ior.sh /lustre
0.ip-0A020804
[hpcadmin@ip-0A020804 ~]$ qstat
Job id            Name             User              Time Use S Queue
----------------  ---------------- ----------------  -------- - -----
...
```
[hpcadmin@ip-0A020804 ~]$ sbatch -N 1 --exclusive -o ior.o%A -J ior /apps/ior/ior.sh /lustre
