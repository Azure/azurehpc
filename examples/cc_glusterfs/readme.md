...
Then copy the `init.sh`,`variables.json`, and scripts directory from `examples/cc_glusterfs` to your working directory.
...
To start the PBS cluster attached to GlusterFS:
...
To start the Slurm cluster attached to GlusterFS:
...
## Step 2 - Check that GlusterFS and NFS shared are mounted

```
[hpcadmin@ip-0A020804 ~]$ df
Filesystem             1K-blocks      Used   Available Use% Mounted on
devtmpfs                16452148         0    16452148   0% /dev
tmpfs                   16463832         0    16463832   0% /dev/shm
tmpfs                   16463832      9364    16454468   1% /run
tmpfs                   16463832         0    16463832   0% /sys/fs/cgroup
/dev/sda2               30416376  10400100    20016276  35% /
/dev/sda1                 505580     65244      440336  13% /boot
/dev/sda15                506608     11328      495280   3% /boot/efi
/dev/sdb1               65923564   2150432    60401360   4% /mnt/resource
tmpfs                    3292768         0     3292768   0% /run/user/0
tmpfs                    3292768         0     3292768   0% /run/user/20002
glusterfsmaster:/gv0 14994586112 150084832 14844501280   2% /glusterfs
jumpbox:/share/apps   2146156736     34816  2146121920   1% /apps
jumpbox:/share/data   2146156736     34816  2146121920   1% /data
...
```
[hpcadmin@ip-0A020804 ~]$ sbatch -N 1 --exclusive -o ior.o%A -J ior /apps/ior/ior.sh /glusterfs
