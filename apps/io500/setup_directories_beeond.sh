...
function setup_directories {
  ...
  io500_result_dir=$resultdir

  mkdir -p $workdir $resultdir
  mkdir -p $workdir/{ior_easy,ior_hard,mdt_easy,mdt_hard}

  # for ior_easy.
  sudo beegfs-ctl --mount=/beeond --setpattern --chunksize=$IOR_EASY_CHUNKSIZE --numtargets=$IOR_EASY_STRIPE $workdir/ior_easy

  # stripe across all OSTs for ior_hard, 256k chunksize
  sudo beegfs-ctl --mount=/beeond --setpattern --chunksize=$IOR_HARD_CHUNKSIZE --numtargets=$IOR_HARD_STRIPE $workdir/ior_hard

  # turn off striping and use small chunks for mdtest
  sudo beegfs-ctl --mount=/beeond --setpattern --chunksize=64k --numtargets=1 $workdir/mdt_easy
  sudo beegfs-ctl --mount=/beeond --setpattern --chunksize=64k --numtargets=1 $workdir/mdt_hard
