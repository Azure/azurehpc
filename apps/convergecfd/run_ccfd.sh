...
#PBS -N ConvergeCFD
#PBS -l select=1:ncpus=120:mpiprocs=120:mem=440gb
#PBS -joe

APPLICATION=convergecfd
APP_INSTALL_DIR=${APP_INSTALL_DIR:-/apps}
CCFD_VERSION=${CCFD_VERSION:-3.0.12}
EX_PATH=${EX_PATH:-${APP_INSTALL_DIR}/Convergent_Science/CONVERGE/$CCFD_VERSION/example_cases/Internal_Combustion_Engines/Gasoline_spark_ignition_PFI}
CASE=${CASE:-SI8_engine_premix_SAGE}
MPI=${MPI:-intel} #Intel is the only valid option for now
