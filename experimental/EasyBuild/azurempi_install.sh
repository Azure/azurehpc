...
ml EasyBuild
STACK_PREFIX=$(echo $EBROOTEASYBUILD | cut -d'/' -f-4)
...
grep -q 'include-easyblocks' easybuild.cfg && \
sed -i -e "/^\(include-easyblocks = \)\(.*\)/{s//\1${STACK_PREFIX}/custom_easyblocks/azurempi.py,\2/}" easybuild.cfg || \
sed -i -e "/^\[config\].*/a include-easyblocks = ${STACK_PREFIX}/custom_easyblocks/azurempi.py" easybuild.cfg
